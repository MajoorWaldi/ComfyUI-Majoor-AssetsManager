...
function createElement(tagName) {
  return {
    ...
    querySelectorAll(selector) {
      ...
      return this.childNodes.filter((node) => String(node.className || "").includes("mjr-toast"));
    },
  };
}
...
function readNodeText(node) {
  ...
  const children = Array.isArray(node.childNodes) ? node.childNodes.map((child) => readNodeText(child)).join("") : "";
  return `${own}${children}`;
}
