...
const BUILTIN_PANEL_MESSAGES = Object.freeze([
    ...
    },
    {
        ...
        body: "Discover Majoor ImageOps, a ComfyUI node pack with practical image operation nodes for your workflows.",
        bodyKey: "msg.tip.body.majoorImageOpsNodePack",
        actionLabel: "Open Node Pack",
        actionLabelKey: "label.openNodePack",
        actionUrl: "https://github.com/MajoorWaldi/ComfyUI-Majoor-ImageOps",
    },
    {
        ...
        categoryKey: "msg.category.information",
        level: "info",
        createdAt: Date.parse("2026-05-10T18:30:00Z"),
        body: "Graph Map now has its own documentation page with screenshots and a quick walkthrough. Open the guide to see how to read the workflow map, inspect selected nodes, and use the node detail actions.",
        bodyKey: "msg.tip.body.graphMapGuide",
        actionLabel: "Graph Map Guide",
    ...
    },
    {
        ...
        categoryKey: "msg.category.information",
        level: "info",
        createdAt: Date.parse("2026-05-10T18:45:00Z"),
        body: "MFV now has its own illustrated guide covering compare modes, A/B/C/D pins, streams, node parameters, run/stop, pop-out, and how Graph Map complements the viewer workflow.",
        bodyKey: "msg.tip.body.mfvGuide",
        actionLabel: "MFV Guide",
...
function _toNumber(value: any, fallback = 0) {
...
function _normalizeMessageLevel(value: any) {
...
function _isBuiltinMessageId(id: any) {
    return BUILTIN_MESSAGE_IDS.has(String(id || "").trim());
}

function _normalizeMessage(input: Record<string, any> = {}) {
...
function _readStorageJson(key: any, fallback: any) {
    try {
...
function _writeStorageJson(key: any, value: any) {
    try {
...
function _writeLastReadAt(ts: any) {
    try {
...
function _writeDismissedBuiltinIds(ids: any) {
...
function _dismissBuiltinMessage(id: any) {
...
function _restoreBuiltinMessage(id: any) {
    const key = String(id || "").trim();
    if (!_isBuiltinMessageId(key)) return false;
    const dismissedIds = _readDismissedBuiltinIds();
...
export function ensurePanelMessagesReady(): void {
    _ensureLoaded();
    if (_seedBuiltinMessages()) _emitChanged();
}

export function listPanelMessages(): Record<string, any>[] {
    _ensureLoaded();
    return _messages.map((message) => ({ ...message }));
}

export function getPanelUnreadCount(): number {
    _ensureLoaded();
...
export function getPanelLastReadAt(): number {
    _ensureLoaded();
    return _readLastReadAt();
}

export function markPanelMessagesRead({ upTo = _safeNow() }: { upTo?: number } = {}): void {
    _ensureLoaded();
    _writeLastReadAt(_toNumber(upTo, _safeNow()));
    _emitChanged();
}

export function upsertPanelMessage(message: Record<string, any>): void {
    _ensureLoaded();
...
export function addPanelMessage(message: Record<string, any>): void {
    return upsertPanelMessage(message);
}

export function removePanelMessage(id: string | number): boolean {
    _ensureLoaded();
    const key = String(id || "").trim();
    if (!key) return false;
...
export function clearPanelMessages(): void {
    _ensureLoaded();
    const dismissedBuiltinIds = _readDismissedBuiltinIds();
