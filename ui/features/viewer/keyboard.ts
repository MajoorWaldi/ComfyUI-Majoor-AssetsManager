 ...
 */
function extractPromptText(asset: any) {
...
export function installViewerKeyboard({
    ...
    let ratingDebounceTimer: any = null;
    let ratingPending: any = null;
    ...
    const scheduleRatingUpdate = (assetId: any, rating: any) => {
        clearRatingDebounce();
    ...
    const handleKeyboard = (e: any) => {
        ...
        const stepFrame = async (direction: any) => {
            if (!isSingle) return false;
            ...
            try {
                ...
                try {
                    video.dispatchEvent?.(
                        new CustomEvent("mjr:frameStep", { detail: { direction, time: next } }),
                    );
                } catch (e: any) {
                    console.debug?.(e);
                }
        ...
        const changePlaybackRate = (deltaOrAbsolute: any, { absolute = false } = {}) => {
            try {
                const controls = getControls();
                ...
                if (absolute) {
                    ...
                    if (Number.isFinite(Number(rate))) {
                        ...
                        comfyToast(
                            t("toast.playbackRate", "Playback {rate}x", {
        ...
        switch (e.key) {
            case "1": {
                ...
                consume();
                try {
                    const near = Math.abs((Number(state?.zoom) || 1) - (z as number)) < 0.01;
                    setZoom?.(near ? 1 : z, {
