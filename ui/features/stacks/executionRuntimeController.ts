...
export function createExecutionRuntimeController({
    ...
    let postExecutionWorkTimer: any = null;

    function isJobTrackingDebugEnabled() {
        try {
            if (typeof window === "undefined") return false;
            if ((window as any).__MJR_DEBUG_JOB_TRACKING__ === true) return true;
            return (
    ...
    function debugJobTracking(stage: any, detail: Record<string, any> = {}) {
    ...
    function rememberExecutionStart(promptId: any, timestampMs: any) {
        if (!promptId) return;
    ...
    function dedupeFiles(files: any) {
        recentFiles.prune();
        const fresh = [];
    ...
    function indexRetryDelayMs(attempt: any) {
    ...
    function schedulePostExecutionWork(work: any, delayMs = executionIdleGraceMs) {
    ...
    function shouldRetryIndexResponse(res: any) {
    ...
    function scheduleGenerationIndex(files: any, attempt = 1, meta: Record<string, any> = {}) {
        const payloadFiles = Array.isArray(files) ? files : [];
        if (!payloadFiles.length) return;
        const payload: Record<string, any> = { files: payloadFiles, origin: "generation" };
        ...
        debugJobTracking("index:direct", {
            ...
            attempt,
            files: payloadFiles.map((item: any) => String(item?.filename || item?.filepath || "")),
        });
        post(ENDPOINTS.INDEX_FILES, payload)
            ...
            .catch((error: any) => reportError(error, "executionRuntime.executed.index"));
    }

    function resolveGenerationPromptId(meta: Record<string, any> = {}) {
    ...
    function orphanGenerationKey(files: any) {
        if (!Array.isArray(files) || !files.length) return "";
        return files
            .map((item: any) => String(item?.filepath || item?.filename || ""))
            .filter(Boolean)
            .sort()
            .join("|");
    }

    function enqueueOrphanGenerationIndex(files: any, meta = {}, attempt = 1) {
        const payloadFiles = Array.isArray(files) ? files : [];
        if (!payloadFiles.length) return;
    ...
    function flushOrphanGenerationEntries(promptId: any) {
    ...
    function bufferGenerationIndex(files: any, meta: Record<string, any> = {}, attempt = 1) {
        ...
        debugJobTracking("buffer:add", {
            ...
            files: Array.isArray(files)
                ? files.map((item: any) => String(item?.filename || item?.filepath || ""))
                : [],
        });
        if (!promptId) {
    ...
    function scheduleFinalizeExecutionStacks(jobId: any, delayMs = 900) {
    ...
    function flushBufferedGenerationIndex(jobId: any) {
        ...
        debugJobTracking("buffer:flush", {
            ...
            files: files.map((item: any) => String(item?.filename || item?.filepath || "")),
        });
    ...
    function hasBufferedGenerationFiles(jobId: any) {
        const promptId = String(jobId || "").trim();
    ...
    function prepareLiveAssetEvent(detail: any) {
    ...
    function isRenderableLiveAsset(detail: any) {
    ...
    function handleExecutedEvent(event: any, { appRef }: { appRef: any }) {
        const policy = getExecutionRuntimePolicy();
        ...
        const genTimeMs = startTs ? Math.max(0, Date.now() - (startTs as number)) : 0;
        let sourceNodeType = "";
        try {
            if (sourceNodeId && typeof appRef !== "undefined") {
                const graphNode = findGraphNodeById(getHostRootGraph(appRef), sourceNodeId);
                if (graphNode) {
    ...
    function handleExecutionStart(event: any, { setCurrentJobId }: { setCurrentJobId: any }) {
        const promptId = event?.detail?.prompt_id || event?.detail?.promptId;
    ...
    function handleExecutionEnd(event: any, { setCurrentJobId }: { setCurrentJobId: any }) {
        const policy = getExecutionRuntimePolicy();
        const promptId = event?.detail?.prompt_id || event?.detail?.promptId;
    ...
    function handleNodeOutputsUpdated(nodeOutputs: any) {
        if (!isExecutionGroupingEnabled()) return;
        const files = [];
