...
const getHighlightState = (app: any) => {
...
export const getNodeUnderClientXY = (app: any, clientX: any, clientY: any) => {
...
export const clearHighlight = (app: any, markCanvasDirty: any) => {
    const state = getHighlightState(app);
...
export const ensureComboHasValue = (widget: any, value: any) => {
 ...
 */
const _pickBestPathWidget = (node: any, droppedExt: any, cfg: any) => {
    ...
    const isKnownNode = cfg.knownNodeIncludes.some((p: any) => nodeType.includes(p));
    ...
    for (const w of widgets) {
        ...
        const hasMediaHint = cfg.mediaTerms.some((t: any) => name.includes(t));
        const hasPathHint = _PATH_HINT_TERMS.some((t: any) => name.includes(t));
        if (hasMediaHint && hasPathHint) score += 80;
        if (_PATH_TERMS.some((t: any) => name.includes(t))) score += 35;

        for (const { terms, score: pts } of cfg.extraTerms) {
            if (terms.some((t: any) => name.includes(t))) score += pts;
        }

        if (_OUTPUTY_TERMS.some((t: any) => name.includes(t))) score -= 90;
...
export const pickBestMediaPathWidget = (node: any, payload: any, droppedExt: any) => {
...
export const getInputSlotUnderClientXY = (app: any, node: any, clientX: any, clientY: any) => {
