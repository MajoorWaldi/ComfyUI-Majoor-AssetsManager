...
import { _persistAuthToken } from "./clientAuth.js";
import { normalizeAssetId, pickRootId } from "../utils/ids.js";

export async function setProbeBackendMode(mode: any) {
...
export async function setMetadataFallbackSettings({ image, media }: Record<string, any> = {}) {
...
export async function setVectorSearchSettings(settings: any = true) {
...
export async function setOutputDirectorySetting(outputDirectory: any, options: Record<string, any> = {}) {
...
export async function setIndexDirectorySetting(indexDirectory: any, options: Record<string, any> = {}) {
...
export async function setSecuritySettings(prefs: any) {
...
export async function openInFolder(assetOrId: any) {
...
export async function resetIndex(options: Record<string, any> = {}) {
    const _bool = (value: any, fallback: any) =>
...
export async function setWatcherScope({ scope = "output", customRootId = "" }: Record<string, any> = {}) {
    const s =
        String(scope || "output")
            .trim()
            .toLowerCase() || "output";
    const rid = String(customRootId || "").trim();
    const body: Record<string, any> = { scope: s };
    if (rid) body.custom_root_id = rid;
    return post(ENDPOINTS.WATCHER_SCOPE, body);
}

export async function getWatcherStatus(options: Record<string, any> = {}) {
...
export async function getToolsStatus(options: Record<string, any> = {}) {
    return get(ENDPOINTS.TOOLS_STATUS, options);
}

export async function getRuntimeStatus(options: Record<string, any> = {}) {
...
export async function listDbBackups(options: Record<string, any> = {}) {
...
export async function restoreDbBackup({ name = "", useLatest = false }: Record<string, any> = {}) {
    const body: Record<string, any> = {};
...
export async function mergeDuplicateTags(keepAssetId: any, mergeAssetIds = []) {
...
export async function deleteAsset(assetOrId: any) {
    let id, payload;
    if (assetOrId && typeof assetOrId === "object") {
...
export async function deleteAssets(assetIds: any) {
...
function _emitAssetsDeleted(ids: any) {
    try {
...
export async function renameAsset(assetOrId: any, newName: any) {
    let id;
    if (assetOrId && typeof assetOrId === "object") {
        id = normalizeAssetId(assetOrId.id);
        const fp = String(
...
export async function createCollection(name: any) {
...
export async function addAssetsToCollection(collectionId: any, assets: any) {
    const id = String(collectionId || "").trim();
    const list = Array.isArray(assets) ? assets : [];
    return post(`/mjr/am/collections/${encodeURIComponent(id)}/add`, { assets: list });
}

export async function removeFilepathsFromCollection(collectionId: any, filepaths: any) {
    const id = String(collectionId || "").trim();
    const list = Array.isArray(filepaths) ? filepaths : [];
    return post(`/mjr/am/collections/${encodeURIComponent(id)}/remove`, { filepaths: list });
}

export async function getCollectionAssets(collectionId: any) {
    const id = String(collectionId || "").trim();
...
export async function vectorSearch(query: any, topKOrOptions: number | Record<string, any> = 20) {
    ...
    const opts =
        topKOrOptions && typeof topKOrOptions === "object"
            ? topKOrOptions
            : { topK: Number(topKOrOptions) } as Record<string, any>;
 ...
 * @returns {Promise<ApiResult<{asset_id:number, score:number}[]>>}
 */
export async function vectorFindSimilar(assetId: any, topKOrOptions: number | Record<string, any> = 20) {
 ...
 */
export async function vectorGetAlignment(assetId: any) {
    const id = String(assetId || "").trim();
    if (!id) return { ok: false, error: "Missing asset ID" };
 ...
 */
export async function vectorIndexAsset(assetId: any) {
    const id = String(assetId || "").trim();
    if (!id) return { ok: false, error: "Missing asset ID" };
 ...
 */
export async function vectorBackfill(batchSize = 64, options: Record<string, any> = {}) {
 ...
 */
export async function vectorGetAutoTags(assetId: any) {
    const id = String(assetId || "").trim();
    if (!id) return { ok: false, error: "Missing asset ID" };
 ...
 * @returns {Promise<ApiResult<string>>}
 */
export async function vectorGenerateCaption(assetId: any) {
    const id = String(assetId || "").trim();
    if (!id) return { ok: false, error: "Missing asset ID" };
 ...
 */
export async function vectorGenerateEnhancedPrompt(assetId: any) {
