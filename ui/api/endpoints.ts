...
export interface ListURLParams extends AssetFilterParams {
    q?: string;
    limit?: number;
    offset?: number;
    scope?: string;
    customRootId?: string | null;
    resolutionCompare?: string | null;
    sort?: string | null;
    cursor?: string | null;
 ...
 */
export function buildViewURL(filename: any, subfolder: string | null = null, type = "output"): string {
 ...
 */
export function buildListURL(params: ListURLParams = {}): string {
    const {
        q = "*",
...
export function buildCustomViewURL(filename: any, subfolder = "", rootId = ""): string {
    const fn = String(filename || "").trim();
...
export function buildBatchZipDownloadURL(token: any): string {
    return `${ENDPOINTS.BATCH_ZIP_CREATE}/${encodeURIComponent(String(token || ""))}`;
}

export function buildStacksURL({ limit = 50, offset = 0, includeTotal = true }: { limit?: number; offset?: number; includeTotal?: boolean } = {}): string {
...
export function buildStackURL(stackId: any): string {
...
export function buildNodeContextMembersURL(
    sourceNodeId: any,
    options: { jobId?: string; job_id?: string; latest?: boolean; limit?: number } = {},
): string {
...
export function buildStackCoverURL(stackId: any): string {
    return `${buildStackURL(stackId)}/cover`;
}

export function buildStackRenameURL(stackId: any): string {
    return `${buildStackURL(stackId)}/rename`;
}

export function buildViewerResourceURL(assetOrContext: any, relpath = ""): string {
    const ctx = (assetOrContext && typeof assetOrContext === "object" ? assetOrContext : {}) as Record<string, any>;
...
export function buildDateHistogramURL(params: AssetFilterParams & { scope?: string; customRootId?: string | null; month?: string } = {}): string {
    const {
        scope = "output",
        customRootId = null,
        month = "",
...
export function buildAssetViewURL(asset: Record<string, any> | null | undefined): string {
    const mtime = asset?.mtime;
    const withMtime = (url: any) => {
    ...
    const pickFromPath = (p: any) => {
        const out = { type: "", subfolder: "", filename: "" };
        const normalized = toPosixPath(p);
        if (!normalized) return out;
        const lower = normalized.toLowerCase();
        const idxOut = lower.indexOf("/output/");
        const idxIn = lower.indexOf("/input/");
        const idxTemp = lower.indexOf("/temp/");
        let baseIdx = -1;
        if (idxOut >= 0) {
            out.type = "output";
 ...
 */
export function buildDownloadURL(filepath: any, options: Record<string, any> = {}) {
    if (!filepath) return "";
 ...
 */
export function buildCleanDownloadURL(filepath: any) {
    if (!filepath) return "";
