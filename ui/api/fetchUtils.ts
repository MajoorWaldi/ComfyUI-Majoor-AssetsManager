...
function _methodIsWrite(method: any) {
...
function _normalizeUrlPath(url: any) {
    try {
...
function _isMajoorApiUrl(url: any) {
    return _normalizeUrlPath(url).startsWith(API_PREFIX);
}

function _isBootstrapTokenUrl(url: any) {
    return _normalizeUrlPath(url) === BOOTSTRAP_TOKEN_PATH;
}

function _isRetryableError(error: any) {
    try {
...
function _buildTimedSignal(options: Record<string, any> = {}) {
...
function _buildPendingRequestKey(method: any, url: any, options: Record<string, any> = {}) {
...
function _deduplicatedFetch(key: any, fetcher: any) {
...
async function _handleResponse(
    ...
    method: any,
    authRetryDone: any,
    { ensureWriteAuthToken, normalizeWriteAuthFailure, fetchAPI, options, retryCount }: Record<string, any>,
) {
...
export function createApiFetchClient({
    ...
    normalizeWriteAuthFailure = (result: any) => result,
    trackApiCall = null,
}: {
    readObsEnabled?: () => boolean;
    readAuthToken?: () => string;
    ensureWriteAuthToken?: (...args: any[]) => Promise<string>;
    normalizeWriteAuthFailure?: (result: any) => any;
    trackApiCall?: ((duration: number, failed: boolean) => void) | null;
} = {}) {
    async function fetchAPI(url: string, options: Record<string, any> = {}, retryCount = 0) {
        ...
        try {
            ...
            const fetchOptions: Record<string, any> = { ...options, headers, signal: timed.signal };
            try {
            ...
            try {
                ...
                } else if (typeof window !== "undefined" && (window as any).MajoorMetrics) {
                    (window as any).MajoorMetrics.trackApiCall(duration, !result?.ok);
                }
            } catch (e) {
                console.debug?.(e);
            }
    ...
    async function post(url: string, body: unknown, options: Record<string, any> = {}) {
        return fetchAPI(url, {
            ...options,
            method: "POST",
