...
export function _persistAuthToken(token: any) {
    const normalized = String(token || "").trim();
...
export function setRuntimeSecurityToken(token: any) {
    const normalized = String(token || "").trim();
    if (!normalized) return false;
...
function _rememberAuthBootstrapFailure(details: Record<string, any> = {}) {
...
function _buildWriteAuthErrorMessage(result: any) {
...
function _notifyWriteAuthFailure(message: any) {
...
export function normalizeWriteAuthFailure(result: any) {
